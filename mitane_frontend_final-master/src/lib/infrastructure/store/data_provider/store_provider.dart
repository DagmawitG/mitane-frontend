...
class StoreProvider {
  ...
  Future<List> getSelfProduct() async {
    try {
      dio.options.headers['content-Type'] = 'application/json';
      dio.options.headers['authorization'] =
          'Bearer ${await storage.read(key: 'token')}';

      final response = await dio.get("$storeUrl/self/product");
      return response.data;
    } catch (e) {
      rethrow;
    }
  ...
  Future<List<StoreProduct>> getStoreProducts(String query) async {
    try {
      ...
      final response = await dio.get("$storeUrl/sorted/products?$query");
      return (response.data['data'] as List)
          .map((u) => StoreProduct.fromJson(u))
          .toList();
    } catch (e) {
      print(e);
      rethrow;
  ...
  Future<List<StoreIngrident>> getStoreIngridents(String query) async {
    try {
      ...
      final response = await dio.get("$storeUrl/sorted/ingredient?$query");
      return (response.data['data'] as List)
          .map((u) => StoreIngrident.fromJson(u))
          .toList();
    } catch (e) {
      print(e);
      rethrow;
  ...
  Future<List<StoreMachinery>> getStoreMachinery(String query) async {
    try {
      ...
      final response = await dio.get("$storeUrl/sorted/machinery?$query");
      return (response.data['data'] as List)
          .map((u) => StoreMachinery.fromJson(u))
          .toList();
    } catch (e) {
      print(e);
      rethrow;
  ...
  Future<bool> createProductItem(ProductItem item, File? image) async {
    try {
      ...
      final response = await dio.post('$storeUrl/add_product', data: formData);
  ...
  Future<bool> createMachineryItem(MachineryItem item, File? image) async {
    try {
      ...
      final response =
          await dio.post('$storeUrl/add_machinery', data: formData);
  ...
  Future<bool> createIngridentItem(IngridentItem item, File? image) async {
    try {
      dio.options.headers['content-Type'] = 'application/json';
      dio.options.headers['authorization'] =
          'Bearer ${await storage.read(key: 'token')}';
      ...
      FormData formData = FormData.fromMap(jsonFormData);
      print("createIngridentItem $jsonFormData");
      final response =
          await dio.post('$storeUrl/add_ingredient', data: formData);
      ...
      print("Unsuccessful creation of ingredient item ${response.data}");
      return false;
    } on DioError catch (e) {
      print(e.response);
      throw e;
...
// Products, ingredients, machineries for the home page
  ...
  Future<bool> updateStoreItem(String type, String store, String productId,
      String storeitem, double price, int quantity, image) async {
    try {
      dio.options.headers['content-Type'] = 'application/json';
      dio.options.headers['authorization'] =
          'Bearer ${await storage.read(key: 'token')}';
      ...
      print("updateStoreItem $image");

      FormData formData = FormData.fromMap(jsonFormData);
      final response = await dio.put("$storeUrl/$type/$store", data: formData);
  ...
  Future<List> getSelfMachinery() async {
    try {
      dio.options.headers['content-Type'] = 'application/json';
      dio.options.headers['authorization'] =
          'Bearer ${await storage.read(key: 'token')}';

      final response = await dio.get("$storeUrl/self/machinery");
      return response.data;
    } catch (e) {
      rethrow;
    }
  ...
  Future<List> getSelfIngrident() async {
    try {
      dio.options.headers['content-Type'] = 'application/json';
      dio.options.headers['authorization'] =
          'Bearer ${await storage.read(key: 'token')}';

      final response = await dio.get("$storeUrl/self/ingredient");
      return response.data;
    } catch (e) {
      rethrow;
    }
